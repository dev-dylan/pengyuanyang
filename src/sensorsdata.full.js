...
//            delete sd.pluginTempFunction[k];
  ...
  sd.bind = function(itemName, itemValue) {
    // 如果是数字，转成字符串
    if (_.isNumber(itemValue)) {
      itemValue = String(itemValue);
    }
    // 检查 key, value 是否字符串并不为空，否则打印错误信息，停止执行
    if (_.isString(itemName) === false || _.isString(itemValue) === false) {
      sd.log('bind参数错误');
      return false;
    }
    itemName = _.trim(itemName);
    itemValue = _.trim(itemValue);
    ...
    /*
    if (itemName === '$login_id' && (sd.store._state.identities.hasOwnProperty('$login_id') === false || sd.store._state.identities['$login_id'] !== itemValue) ) {
      sd.store.identities.reset();
    }
    */
    var reservedNames = ['$login_id', '$anonymous_id', '$cookie_id'];
    if (_.indexOf(reservedNames, itemName) > -1) {
      sd.log('bind参数错误，不支持绑定' + itemName);
      return false;
    }
  ...
  sd.unbind = function(itemName, itemValue) {
    ...
    var reservedNames = ['$anonymous_id', '$cookie_id', '$login_id'];
    if (_.indexOf(reservedNames, itemName) > -1) {
      sd.log('unbind参数错误，不支持绑定' + itemName);
      return false;
    }
  ...
  sd.login = function(id, callback){
    ...
    if (saEvent.check({distinct_id: id})) {
      ...
      var isNewLoginId = sd.store._state.identities.hasOwnProperty('$login_id') === false || id !== sd.store._state.identities.$login_id;
      ...
      if(id !== distinctId){
        ...
        // 3.0 新增
        if (isNewLoginId) {
          sd.store._state.identities.$login_id = id;
        }
  ...
  var store = sd.store = {
    identities: {
      set: function(name, id) {
        ...
        switch(name) {
          case 'login':
            identities.$login_id = id;
            identities.$cookie_id = sd.store._state.identities.$cookie_id;
            break;
          case 'logout':
            identities.$cookie_id = sd.store._state.identities.$cookie_id;
            break;
    ...
    init: function() {
      ...
      if (cross === null) {
        ...
        this.set('identities',{$cookie_id:uuid});
        ...
        var state = null;
        if (ds != null && _.isJSONString(ds)) { // cookie 有值
          state = JSON.parse(ds);
          this._state = _.extend(state);


          // identities 不存在的话
          var unionId = store.getUnionId();
          if (!_.isObject(sd.store._state.identities)) {
            sd.store._state.identities = {};
            sd.store._state.identities.$anonymous_id = unionId.anonymous_id;
            sd.store._state.identities.$cookie_id = _.UUID();
          }
          if (sd.store._state.identities.hasOwnProperty('$login_id')  === false && unionId.login_id) {
            sd.store._state.identities.$login_id = unionId.login_id;
          }
          sd.store.save();



          if (state.distinct_id) { // 存在 distinct_id
          ...
          } else { // 不存在 distinct_id
            var newuuid = _.UUID();
            this.set('distinct_id', newuuid);
            this.set('identities',{$cookie_id: newuuid});
