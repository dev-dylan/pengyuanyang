...
class OpusOggConverter {
    ...
    /**
     * 读取裸 Opus 数据（每帧40字节，无头）
     * @param {string} inputFile - 输入文件路径
     * @returns {Buffer[]} - 音频帧数组（每帧40字节）
     */
    readRawOpusData(inputFile) {
        const frames = [];
        const frameSize = 40; // 每帧40字节
        ...
        try {
            ...
            const totalFrames = Math.floor(fileSize / frameSize);
            const fileBuffer = fs.readFileSync(inputFile);

            for (let i = 0; i < totalFrames; i++) {
                const offset = i * frameSize;
                const frameData = fileBuffer.slice(offset, offset + frameSize);
                
                if (frameData.length === frameSize) {
                    frames.push(frameData);
                }
            }
            console.log(`成功读取 ${frames.length} 帧裸 Opus 数据`);
            console.log(`  每帧大小: ${frameSize} 字节`);
...
function main() {
    ...
    if (args.length > 0) {
        ...
        if (!path.isAbsolute(testInput)) {
            testInput = path.join(process.cwd(), testInput);
        }
    } else {
        // 默认输入文件
        testInput = path.join(__dirname, "raw_test_1.opus");
    }

    // converter.readCustomFormat(testInput);
    // 生成输出文件名
    const inputBasename = path.basename(testInput, path.extname(testInput));
    const testOutput = path.join(outputDir, `${inputBasename}_output.opus`);

    // 检查输入文件
    if (!fs.existsSync(testInput)) {
        console.error(`错误: 输入文件不存在: ${testInput}`);
        console.log("\n使用方法:");
        console.log("  node test_converter.js <输入文件路径>");
        console.log("\n示例:");
        console.log("  node test_converter.js test1.opus");
        console.log("  node test_converter.js /path/to/raw_opus_data.opus");
        return;
    }
